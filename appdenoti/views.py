...
def index(request):
    ...
    if usuario.is_authenticated:
        if usuario.is_staff:
            ...
            entidade = Entidade.objects.get(usuario__id=usuario.id)
...
def buscausuario(request):
    dados = {}
    form = buscafiltro(request.GET or None)
    all_users = ModelUsuario.objects.all()
    ...
    if form.is_valid():
        ...
        if search_input:
            ...
            if fiscal_match:
                ...
                queries.append(Q(fiscal__first_name__icontains=fiscal))
            ...
            if entidade_match:
                ...
                queries.append(Q(fiscal__first_name__icontains=form.cleaned_data['nome']))
...
def generate_pdf(request, codigo_verificador):
    ...
    pdf_template_path  = os.path.join(settings.BASE_DIR, 'appdenoti/static/modelonotif.pdf')
...
def filtrosue(request):
    ...
    paginator = Paginator(notificacoes, 10) 
    page_number = request.GET.geeracaoFot('page')
    data['db'] = paginator.get_page(page_number)
...
@login_required
@user_passes_test(is_staff)
def listaliberacao(request):
    dados = {}
    form = buscafiltro(request.GET or None)
    all =  Liberacao.objects.all()
    ...
    if form.is_valid():
        ...
        if search_input:
            email_match = re.search(r'email:\s*([^\s]+)', search_input, re.IGNORECASE)
          
            
            if email_match:
                email = email_match.group(1)
                queries.append(Q(Liberacao__email__icontains=email))
    ...
    return render(request, 'listaliberacao.html', dados)
